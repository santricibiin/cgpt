...
func RunBatch(totalAccounts int, outputFile string, maxWorkers int, proxy, defaultPassword, defaultDomain string) {
	RunBatchWithProgress(totalAccounts, outputFile, maxWorkers, proxy, defaultPassword, defaultDomain, nil)
}

// RunBatchWithProgress is like RunBatch but calls onProgress(successCount, total) after each success.
func RunBatchWithProgress(totalAccounts int, outputFile string, maxWorkers int, proxy, defaultPassword, defaultDomain string, onProgress func(int, int)) {
		...
		go func(workerID int) {
			...
			for {
				...
				if success {
					...
					printMu.Unlock()
					if onProgress != nil {
						onProgress(int(sc), totalAccounts)
					}
				} else {
